...
CdS_temp %>% dplyr::mutate(start = as.Date(start), end = as.Date(end)) %>%
  dplyr::mutate(Censored = ifelse(end == "2017-01-01", 1, 0))


# Change ids so they span the number of unique ids - use counter instead of id
CdS_UniqueId <- CdS_temp %>%
  dplyr::group_by(MediCirqueId) %>%
  summarise(max = max(total_missed_perfo)) %>%
  ungroup() %>%
  arrange(MediCirqueId) %>%
  dplyr::mutate(counter = row_number())

CdS_temp2 <- left_join(CdS_UniqueId, CdS_temp, by = "MediCirqueId") %>%
  dplyr::select(MediCirqueId, PHCInjuryId, counter, start, end, missed_days, total_missed_perfo, Censored) %>%
  dplyr::mutate(Injury_Length = total_missed_perfo)

# Calculate max length of injury for each injury (since sometimes they come back...?)
CdS_UniqueInjury <- CdS_temp2 %>%
  dplyr::group_by(PHCInjuryId) %>%
  dplyr::summarise(max_missed_perfo = max(total_missed_perfo), n = n())

# this give you doubles!
CdS_temp3 <- left_join(CdS_UniqueInjury, CdS_temp2, by =c("PHCInjuryId"="PHCInjuryId", "max_missed_perfo" = "Injury_Length"))
...
CdS_number <- CdS %>%
  dplyr::filter(Censored == 1)

pdf("Return_PeriodAll.pdf",width=5, height=5)
mar.default <- c(5,4,4,2) + 0.1
ggplot(data = CdS, aes(y = Injury_Length, x =Injury_Index))  + geom_jitter( col="black") +
  xlab("Injury Id") + ylab("Number of Performances Missed Due to Injury")  +
  geom_vline(xintercept = seq(from = 285, to = 2800, by = 285), linetype = "dotted") +
  scale_x_continuous(limits = c(-1, 2700)) + geom_hline(yintercept = 500)+ theme_bw() +
  geom_text(data=data.frame(x=0,y=500), aes(x, y), label="r=500", vjust=-0.8, hjust = -0.1)
dev.off()

# Cut off greater than 923 (3 years?)
CdS_927 <- CdS %>% dplyr::filter(start <= "2014-01-01")

ggplot(data = CdS_927, aes(y = Injury_Index, x =Injury_Length))  + geom_jitter( col="black") +
  ylab("Injury id (number of injuries observed)") + xlab("Number of Performances Missed due to an Injury")  +  geom_vline(xintercept = 926, linetype = "dotted") +
  geom_hline(yintercept = seq(from = 0, to = 20000, by = 5000))+ theme_bw() +  ggtitle("5000-Observation Return Level")


pdf("Return_LevelAll.pdf",width=5, height=5)
mar.default <- c(5,4,4,2) + 0.1
ggplot(data = CdS, aes(y = Injury_Index, x =Injury_Length))  + geom_jitter( col="black") +
  ylab("Injury id (number of injuries observed)") + xlab("Number of Performances Missed due to an Injury")  +
  geom_vline(xintercept = 589, linetype = "dotted") +
  geom_hline(yintercept = seq(from = 0, to = 2500, by = 500))+ theme_bw() +  ggtitle("500-Observation Return Level")+
...
p1 <- ggplot(data = CdS, aes(y = total_missed_perfo, x = counter)) +
  ...
  xlab("Artist Id") +
  ylab("Number of Performances Missed Due to Injury")+ theme_bw()

p2 <- ggplot(data = CdS, aes(total_missed_perfo)) +
  geom_histogram(binwidth = 10, col = "black", fill = "white", alpha = .2) +
  xlab("Number of Performances Missed due to Injury") + ylab("Count")
...
threshold = 100
data <- CdS
data_u <- data %>% dplyr::filter(Injury_Length > 100) %>%
  dplyr::mutate(Injury_Length = Injury_Length - 100)

data_u_cens <- data_u %>% dplyr::filter(Censored == 1)

CdS_ties <- data_u %>%
  dplyr::group_by(Injury_Length) %>%
  dplyr::summarise(n = n())
...
for(i in 1:length(method)){
  met_all[1,2*i - 1] <-  mle(data = data, threshold = threshold, method = method[i], information = "observed")$par.ests[1]
  met_all[1,i+i] <-  mle(data = data, threshold = threshold, method = method[i], information = "observed")$par.ses[1]
  met_all[2,2*i - 1] <-  mle(data = data, threshold = threshold, method = method[i], information = "observed")$par.ests[2]
  met_all[2,i+i] <-  mle(data = data, threshold = threshold, method = method[i], information = "observed")$par.ses[2]
}
met_all
...
gpd_extRemes_exp <- extRemes::fevd(x = data$total_missed_perfo, threshold = 100, type = "Exponential")
...
beta_reg <- as.numeric(extRemes::fevd(x = data$total_missed_perfo, threshold = 100, type = "Exponential")$results$par)
...
ggplot(data = CdS_950, aes(y = Injury_Index, x =Injury_Length)) +
  geom_jitter( col="black") +
  ylab("Injury Id") + xlab("Number of Performances Missed Due to Injury")  +
  geom_vline(xintercept = 950, linetype = "dotted") +
  geom_hline(yintercept = c(5000, 10000))+ theme_bw() +
...
cirque_max <- CdS %>%
  group_by(counter) %>%
  summarise(max_missed_perfo = max(total_missed_perfo), min_start = min(start))

CdS.2 <- left_join(CdS, cirque_max, by = "counter") %>%
  dplyr::mutate(Longest_Injury = ifelse(max_missed_perfo == total_missed_perfo, 1, 0)) %>%
  dplyr::mutate(Longest_Injury = as.factor(Longest_Injury))
...
tmp <- CdS %>% dplyr::group_by(MediCirqueId) %>%
  dplyr::summarise(Injury_Length_max = max(total_missed_perfo))

data_max_double <- left_join(tmp, CdS, by = c("MediCirqueId"="MediCirqueId", "Injury_Length_max"="total_missed_perfo"))
...
ggplot(data = data_max, aes(y = Injury_Length, x =counter))  +
  geom_jitter( col="black") +
  xlab("Artist id") + ylab("Number of Performances Missed due to Injury")  +
  geom_vline(xintercept = seq(from = 0, to = 240, by = 40), linetype = "dotted") +
  scale_x_continuous(limits = c(0, 240)) + geom_hline(yintercept = 500)
dev.off()
...
data_max_u <- data_max %>%
  dplyr::filter(Injury_Length_max > threshold) %>%
...
for(i in 1:length(method)){
  met_all_max[1,2*i - 1] <-  mle(data = data, threshold = threshold, method = method[i], information = "observed")$par.ests[1]
  met_all_max[1,i+i] <-  mle(data = data, threshold = threshold, method = method[i], information = "observed")$par.ses[1]
  met_all_max[2,2*i - 1] <-  mle(data = data, threshold = threshold, method = method[i], information = "observed")$par.ests[2]
  met_all_max[2,i+i] <-  mle(data = data, threshold = threshold, method = method[i], information = "observed")$par.ses[2]
}
met_all_max
...
data337 <- data %>%
  dplyr::filter(total_missed_perfo > 337) %>%
  group_by(MediCirqueId) %>%
  dplyr::summarise(n = n()) %>%
  dplyr::filter(n > 1)
...
pmax <- ggplot(data = CdS.2, aes(y = total_missed_perfo, x = counter, color = Longest_Injury)) +
  ...
  xlab("Artist Id") + ylab("Total Missed Performances")
...
pmax_u <- ggplot(data = CdS.2, aes(y = total_missed_perfo, x = counter, color = Longest_Injury)) +
  ...
  xlab("Artist Id") +
  ylab("Number of Performances Missed Due to Injury") +
  geom_hline(yintercept = c(88, 337), col = "black") + theme_bw()
...
CdS.81 <- CdS %>%
  dplyr::filter(total_missed_perfo >= 81) %>%
  group_by(counter) %>% summarise(n = n()) %>%
  dplyr::filter(n >=2)

CdS.12 <- CdS %>%
  dplyr::filter(counter == 12)
...
ggplot(data = CdS.12, aes(y = total_missed_perfo, x = start)) +
  ...
  geom_vline(xintercept = c(as.Date("2008-06-8"),
                            ...
                            as.Date("2012-10-15"),
                            as.Date("2013-03-31"),
                            as.Date("2014-03-02")), linetype = "dotted") +
  xlab("Date")

CdS.323 <- CdS %>%
  dplyr::filter(total_missed_perfo >= 323) %>%
  group_by(counter) %>%
  summarise(n = n()) %>%
  dplyr::filter(n >=2)
...
cirque_when <- left_join(cirque_max, CdS, by = c("MediCirqueId"="MediCirqueId", "max_missed_perfo"="total_missed_perfo")) %>%
  dplyr::mutate(diff = start - min_start) %>%
  dplyr::select(MediCirqueId, max_missed_perfo, min_start, start, diff)
...
cirque_month <- CdS %>%
  dplyr::mutate(month = format(start, "%Y/%m"))  %>%
  dplyr::group_by(month) %>%
  summarise(mean = mean(total_missed_perfo))

cirque_year <- left_join(cirque_max, CdS, by = c("MediCirqueId"="MediCirqueId", "max_missed_perfo"="total_missed_perfo")) %>%
  select(MediCirqueId, max_missed_perfo, start) %>%
  dplyr::mutate(year = format(start, "%Y"))  %>%
...
cirque_year_num <- cirque_year %>%
  dplyr::group_by(year) %>%
  dplyr::summarise(n = n())

pdf("cirque_year.pdf", width = 5, height = 5)
mar.default <- c(5, 4, 4, 2) + 0.1
barplot(cirque_year_num$n, xlab = "Year", names.arg=c("2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016"))
abline(h = 2356)
...
p7 <- ggplot(data = cirque_year, aes(y = total_missed_perfo, x = year)) +
  ...
  xlab("Year") + ylab("Maximum Number Missed Performances")
