gen_data <- function(censor, xi, n, num_inj, beta, rate_exp){
  ...
  Cens_index_long <- as.vector(matrix(t(Censored_Matrix_inj), nrow = n*num_inj, ncol = 1))
  ...
  full <- full %>% dplyr::mutate(Perc_Cens = sum(Censored)/nrow(full))

  #   Injury_Length ID Censored Perc_Cens
  # 1     0.2211169  1        0      0.15
  # 2     1.1256608  1        0      0.15
  # 3     0.8824792  1        0      0.15
  # 4     0.4047432  1        0      0.15
  # 5     0.3404012  1        1      0.15 # CORRECT
  # 6     2.0664441  2        0      0.15
...
mle <- function(data,  method = c("MLE_All", "MLE_Delete", "CensMLE")){
  method <- match.arg(method)
  if(method == "MLE_All"){
    mlegpd2(data)
  }
  else if(method == "MLE_Delete"){
    data_nc <- data %>% dplyr::filter(Censored==0)
    mlegpd2(data_nc)
  }
  else{
    mlecensgpd2(data)
